...
int main(int argc, char **argv) {
  ...
  std::vector<std::string> instr_file_names = {"add_one_insts.txt", "add_four_insts.txt", "add_ten_insts.txt"};
  std::vector<std::string> pdi_file_names = {"add_one.pdi", "add_four.pdi", "add_ten.pdi"};
  assert(instr_file_names.size() == NUM_KERNELS);
  assert(pdi_file_names.size() == NUM_KERNELS);

  // Prepending the sourcePath to each file
  for(int i = 0; i < NUM_KERNELS; i++) {
    instr_file_names[i] = sourcePath / instr_file_names[i];
    pdi_file_names[i] = sourcePath / pdi_file_names[i];
  }

  // Structures to store the metadata for all kernels 
  std::vector<uint32_t *> instr_buf(NUM_KERNELS);
  std::vector<uint32_t>   num_instrs(NUM_KERNELS);
  std::vector<uint32_t>   instr_handles(NUM_KERNELS);
  std::vector<uint64_t *> pdi_buf(NUM_KERNELS);
  std::vector<uint32_t>   pdi_handles(NUM_KERNELS);
  ...
  // Used to store all PDIs that we want to launch using this queue
  hsa_amd_aie_ert_hw_ctx_cu_config_t *cu_config = (hsa_amd_aie_ert_hw_ctx_cu_config_t *)malloc(NUM_KERNELS * sizeof(hsa_amd_aie_ert_hw_ctx_cu_config_t));

  for(int i = 0; i < NUM_KERNELS; i++) {

    load_instr_file(global_dev_mem_pool, instr_file_names[i],
                  reinterpret_cast<void **>(&(instr_buf[i])), num_instrs[i]);
    r = hsa_amd_get_handle_from_vaddr(instr_buf[i], &instr_handles[i]);
    assert(r == HSA_STATUS_SUCCESS);
    assert(instr_handles[i] != 0);

    load_pdi_file(global_dev_mem_pool, pdi_file_names[i],
                  reinterpret_cast<void **>(&(pdi_buf[i])));
    r = hsa_amd_get_handle_from_vaddr(pdi_buf[i], &pdi_handles[i]);
    assert(r == HSA_STATUS_SUCCESS);
    assert(pdi_handles[i] != 0);

    cu_config[i].cu_config_bo = pdi_handles[i];
    cu_config[i].cu_func = 0;

  }

  hsa_amd_aie_ert_hw_ctx_config_cu_param_t config_cu_args{
      .num_cus = NUM_KERNELS, .cu_configs = cu_config};
  ...
  std::vector<uint32_t *> input(NUM_KERNELS);
  std::vector<uint32_t *> output(NUM_KERNELS);
  std::vector<uint32_t> input_handle(NUM_KERNELS);
  std::vector<uint32_t> output_handle(NUM_KERNELS);
  std::vector<hsa_amd_aie_ert_start_kernel_data_t *> cmd_payloads(NUM_KERNELS);

  uint64_t wr_idx = 0;
  uint64_t packet_id = 0;

  for (int pkt_iter = 0; pkt_iter < NUM_KERNELS; pkt_iter++) {
    ...
    // Uses one-hot encoded vector to pick corresponding PDI
    cmd_payload->cu_mask = 1 << pkt_iter;
    // Transaction opcode
    cmd_payload->data[0] = 0x3;
    cmd_payload->data[1] = 0x0;
    cmd_payload->data[2] = instr_handles[pkt_iter];
    cmd_payload->data[3] = 0x0;
    cmd_payload->data[4] = num_instrs[pkt_iter];
  ...
  for (int pkt_iter = 0; pkt_iter < NUM_KERNELS; pkt_iter++) {
    for (std::size_t i = 0; i < num_data_elements; i++) {
      ...
      if(pkt_iter == 0) {
        ...
        expected = *(input[pkt_iter] + i) + 4;
      }
      else if(pkt_iter == 2) {
        expected = *(input[pkt_iter] + i) + 10;
      }
      else {
        std::cerr << "[ERROR] No validation for packet " << pkt_iter << std::endl;
        return 1;
      }
    ...
    // Freeing all memory associated with this kernel
    ...
    assert(r == HSA_STATUS_SUCCESS);
    r = hsa_amd_memory_pool_free(pdi_buf[pkt_iter]);
    assert(r == HSA_STATUS_SUCCESS);
    r = hsa_amd_memory_pool_free(instr_buf[pkt_iter]);
    assert(r == HSA_STATUS_SUCCESS);
  }
